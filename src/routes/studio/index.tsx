...
const AI_STYLES = [
	{ id: 'executive', name: 'Executive Classic', image: '/hero_headshot_preview_1773972472569.png', desc: 'Sharp, professional corporate headshot.' },
	{ id: 'fashion', name: 'High Fashion Editor', image: '/auth_fashion_portrait_1.png', desc: 'Editorial aesthetic with dramatic lighting.' },
	{ id: 'cinematic', name: 'Cinematic Moody', image: '/auth_fashion_portrait_2.png', desc: 'Film-like grade with deep contrast.' },
];
		...
		<div className="relative min-h-[calc(100vh-5rem)] flex flex-col items-center justify-center overflow-hidden p-6 md:p-12">
			...
			<div className="absolute inset-0 z-0 flex flex-wrap gap-4 opacity-30 pointer-events-none p-4 select-none justify-center">
				{[...Array(12)].map((_, i) => (
					...
					>
						<img
							src={i % 3 === 0 ? '/hero_headshot_preview_1773972472569.png' : i % 2 === 0 ? '/auth_fashion_portrait_1.png' : '/auth_fashion_portrait_2.png'}
							alt=""
			...
			<div className="relative z-10 flex flex-col items-center w-full max-w-4xl pt-6">
                ...
                >
					...
					<p className="text-lg text-muted-foreground font-light tracking-wide max-w-lg mx-auto mt-4">
						Reimagine your professional presence with Studio 1.0. Upload a simple selfie, and let our neural networks craft your legacy.
					</p>
				</motion.div>
				...
				<div className="relative w-full max-w-3xl translate-y-6 z-20 flex flex-col sm:flex-row gap-2 px-4 transition-all duration-500">
					<StepCard step="01" title="Upload your image" isActive={step === 1} isCompleted={step > 1} />
					<StepCard step="02" title="Generate styles" isActive={step === 2} isCompleted={step > 2} />
                ...
                >
					<div className="flex justify-between items-center mb-6 px-2 relative z-20">
						<div className="flex items-center gap-2">
							...
							<span className="font-bold tracking-widest text-sm uppercase">Studio 1.0</span>
						</div>
                            ...
                            >
                                ...
                                <div 
                                    ...
                                    className={cn(
                                        ...
                                        isDragging ? "border-primary bg-primary/10 scale-[1.02]" : "border-white/20 hover:border-primary/50"
                                    )}
                                >
                                    ...
                                    <div className="absolute inset-0 opacity-10 group-hover:opacity-20 transition-opacity bg-cover bg-center duration-1000 grayscale mix-blend-overlay blur-xs scale-110 group-hover:scale-100" style={{ backgroundImage: "url('/auth_fashion_portrait_1.png')" }} />
                                    ...
                                    <div className="relative p-12 flex flex-col items-center justify-center text-center space-y-4">
                                        <div className={cn(
                                            ...
                                            isDragging ? "bg-primary text-primary-foreground scale-110" : "bg-background text-white group-hover:scale-110 group-hover:-translate-y-1"
                                        )}>
                                        ...
                                        <div>
                                            <h3 className="text-xl font-bold mb-1">Drag & drop your selfie here</h3>
                            ...
                            >
                                <div className="grid md:grid-cols-5 gap-6">
                                    ...
                                    <div className="md:col-span-2 space-y-3">
                                        <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold px-2">Source Image</div>
                                        <div className="relative rounded-2xl overflow-hidden glass border border-white/10 aspect-square group">
                                            <img src={previewUrl} alt="Upload preview" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                            <button 
                                                onClick={clearFile}
                                    ...
                                    <div className="md:col-span-3 space-y-3">
                                        <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold px-2">Select Studio Direction</div>
                                            ...
                                            {AI_STYLES.map((style) => (
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        selectedStyle === style.id ? "border-primary ring-1 ring-primary/50 shadow-lg shadow-primary/20" : "border-white/10 hover:border-white/30 brightness-75 hover:brightness-100"
                                                    )}
                                                >
                                                    ...
                                                    <div className="absolute inset-0 p-3 flex flex-col justify-end">
                                                        <h4 className="font-bold text-sm text-white mb-0.5">{style.name}</h4>
                                                        <p className="text-[10px] text-white/70 leading-tight line-clamp-2">{style.desc}</p>
                                                    </div>
...
function StepCard({ step, title, isActive = false, isCompleted = false }: { step: string; title: string, isActive?: boolean, isCompleted?: boolean }) {
	return (
		<div className={cn(
            "flex-1 p-4 rounded-xl border flex flex-col justify-center backdrop-blur-xl transition-all duration-500",
            isActive ? "bg-primary/20 border-primary shadow-lg shadow-primary/20 scale-[1.02] z-10" : "bg-background hover:bg-white/5 border-white/10",
